...
% Loop through parameter sets and plot each
...
for k = 1:numCases
    ...
    figure('Name', sprintf('Case %d: \\mu=%.4f, M=%d', k, mu, M), 'NumberTitle','off');
    
    % (1) Time-domain: Desired, Noisy Input, Filtered Output
    subplot(2,1,1);
    ...
    xlabel('Time (s)');
    ylabel('Amplitude');
    title(sprintf('Signals (\\mu=%.4f, M=%d)', mu, M));
    legend('Desired','Noisy Input','Filtered','Location','Best');
    grid on;
    
    % (2) Error magnitude with convergence marker
    subplot(2,1,2);
    plot(t, err,    'k', 'LineWidth',1);
    hold on;
    plot(t, err_s,  'b', 'LineWidth',1.2);
    xlabel('Time (s)');
    ylabel('|s(n)-d(n)|');
    title('Error Signal & Smoothed Error');
    grid on;
    if ~isnan(conv_samples(k))
        xline(conv_samples(k)/fs, '--r', sprintf('Conv @ %d samples', conv_samples(k)));
    end
end
